...
def ask_password(title: str = "输入密码", message: str = "请输入密码：", show_asterisk: bool = True) -> str | None:
    """
    ...
    key_entry = tk.Entry(top, textvariable=key_var, show="*" if show_asterisk else "", width=30)
    ...
    def on_ok():
        nonlocal result
        password_str = key_var.get()
        key_var.set("")
        if show_asterisk:
            result = SecureBytes(password_str)
        else:
            result = password_str
        top.destroy()
